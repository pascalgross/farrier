...
| --- | --- |
| `brand/farrier-mark.svg` | The master. `fill="currentColor"`, so an inline copy takes the surrounding text's colour |
| `web/public/mark.svg` | The control plane's toolbar, loaded with `<img>`, so it carries the colour itself |
...
tab strip, and its dark one, which washes out against a light one.
