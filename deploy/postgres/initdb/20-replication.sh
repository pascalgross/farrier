...
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname postgres \
	-v user="$FARRIER_REPLICATION_USER" -v password="$FARRIER_REPLICATION_PASSWORD" <<-'SQL'
...
# is the project's own network and nothing else. Set FARRIER_REPLICATION_HBA to a CIDR when the standby
...
hba_source="${FARRIER_REPLICATION_HBA:-samenet}"

{
	printf '\n# Added by Farrier: streaming replication and pg_basebackup for the role below.\n'
	printf 'host    replication    %s    %s    scram-sha-256\n' \
		"$FARRIER_REPLICATION_USER" "$hba_source"
} >> "${PGDATA:?}/pg_hba.conf"

echo "farrier: replication role $FARRIER_REPLICATION_USER may connect from $hba_source"
