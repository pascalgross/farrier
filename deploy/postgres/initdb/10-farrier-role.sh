...
# ordinary role that owns its own database. farrier-server checks this for itself at startup and
...
: "${FARRIER_DB_USER:?FARRIER_DB_USER must be set}"
: "${FARRIER_DB_PASSWORD:?FARRIER_DB_PASSWORD must be set}"
: "${FARRIER_DB_NAME:?FARRIER_DB_NAME must be set}"
...
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname postgres \
	-v user="$FARRIER_DB_USER" -v password="$FARRIER_DB_PASSWORD" -v db="$FARRIER_DB_NAME" <<-'SQL'
...
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname postgres -tA \
	-v user="$FARRIER_DB_USER" <<-'SQL'
	SELECT format(
		'farrier: role %I is superuser=%s bypassrls=%s (both must be false, or tenants are not isolated)',
