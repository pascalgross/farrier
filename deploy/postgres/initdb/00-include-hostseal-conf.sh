#!/bin/bash
# Make the generated postgresql.conf read hostseal.conf, from the end.
#
...
#
# This runs once, during the first start, on an empty data directory. Editing hostseal.conf afterwards
...
include="include_if_exists = '/etc/postgresql/hostseal.conf'"
...
if grep -qF "$include" "$conf"; then
	echo "hostseal: postgresql.conf already includes /etc/postgresql/hostseal.conf"
else
	{
		printf '\n# Added by HostSeal: settings that make replication and point-in-time recovery possible\n'
		printf '# without a restart later. The file is mounted read-only from deploy/postgres/hostseal.conf.\n'
		printf "%s\n" "$include"
	} >> "$conf"
	echo "hostseal: postgresql.conf now includes /etc/postgresql/hostseal.conf"
fi
