...
// TestAClaimedCooldownStopsASecondMail is the server half of the atomic cooldown.
//
// The store proves that exactly one caller may claim a (rule, host) pair; this proves the routing
// path honours the answer. It matters because the two deliveries race in a way no single-threaded
// reading of the code shows: a unit failing twice in one heartbeat is two events, each with its own
// goroutine, and before the claim both would have mailed.
//
// The assertion is the delivery record rather than a mail, and that is what makes it observable at
// all: this installation has no relay, so a rule that *is* mailed records why it could not be, and a
// rule that loses the claim records nothing because nothing was attempted.
func TestAClaimedCooldownStopsASecondMail(t *testing.T) {
	h := newHarness(t)
	status, raw := h.adminJSON(t, h.adminToken, http.MethodPost, "/api/v1/alerts", map[string]any{
		"condition": "job_failed", "cooldownSeconds": 3600,
		"emailTo": []string{"oncall@example.com"},
	})
	if status != http.StatusCreated {
		t.Fatalf("creating the rule: %d %s", status, raw)
	...
	// Somebody else got there first — another goroutine in this process, or another control plane.
	won, err := h.scoped().ClaimAlertNotification(
		context.Background(), created.ID, state.HostID, time.Now().UTC(), time.Hour)
	if err != nil || !won {
		t.Fatalf("pre-claiming the cooldown: won=%v err=%v", won, err)
	}
	...
	// The event is in the inbox — the claim bounds who mails, never what is recorded.
	if n := countEvents(h.listEvents(t, "?kind=job.failed"), "job.failed"); n != 1 {
		t.Fatalf("the failure produced %d events", n)
	}

	// And no mail was attempted, so no delivery outcome was written. Given a generous window,
	// because the absence of a write is what is being asserted and a passing test must not be one
	// that simply looked too early.
	...
	for time.Now().Before(deadline) {
		...
		for _, rule := range rules {
			if rule.ID == created.ID && !rule.LastDeliveryAt.IsZero() {
				t.Fatalf("a rule whose cooldown was already claimed still attempted mail: %q",
