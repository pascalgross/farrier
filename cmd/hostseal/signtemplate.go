...
// signTemplateCommand implements `hostseal sign-template`.
//
// It is `hostseal sign` for the Tier 2 bootstrap: the signature is produced offline, by a key the
...
//
// The whole body is printed before signing, the same way `hostseal sign` prints the signed payload
...
func signTemplateCommand(argv []string) int {
	...
	keyPath := fs.String("key", "",
		"the signing key: a path from `hostseal key generate`, or a backend reference")
	...
	if *keyPath == "" || *name == "" || *bodyPath == "" {
		fmt.Fprintln(os.Stderr, "hostseal: --key, --name and --body are all required")
		return 2
	}

	raw, err := os.ReadFile(*bodyPath)
	if err != nil {
		fmt.Fprintf(os.Stderr, "hostseal: reading the body: %v\n", err)
		return 2
	}
	if len(raw) == 0 || len(raw) > provision.MaxBodyBytes {
		fmt.Fprintf(os.Stderr, "hostseal: the body must be between 1 byte and %d bytes\n",
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "hostseal: could not canonicalise the template: %v\n", err)
		return 1
	}
	...
	// `hostseal sign` states: the prompt above is a person reading a template, and a timeout that
	...
	fmt.Fprintf(os.Stderr, "\nSigned. Store it with:\n"+
		"  curl -X POST \"$HOSTSEAL_URL/api/v1/templates\" \\\n"+
