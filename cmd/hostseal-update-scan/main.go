...
// Command hostseal-update-scan reports the updates pending on a Windows host, and does nothing else.
//
...
import (
	...
	"github.com/pascalgross/hostseal/internal/wua"
)
...
func main() {
	...
	if flag.NArg() != 0 {
		fmt.Fprintln(os.Stderr, "hostseal-update-scan takes no arguments")
		os.Exit(2)
	}
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "hostseal-update-scan: encoding the result: %v\n", err)
		os.Exit(1)
	}
	if _, err := os.Stdout.Write(append(raw, '\n')); err != nil {
		fmt.Fprintf(os.Stderr, "hostseal-update-scan: writing the result: %v\n", err)
		os.Exit(1)
	}
}
