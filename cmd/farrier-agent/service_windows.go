...
//
// It matches the name packaging/windows/Install-FarrierAgent.ps1 creates, and the virtual account the
// installer runs it as is derived from it: NT SERVICE\farrier-agent. The two must agree, because the
// account exists only as a consequence of the service being registered under this name.
const ServiceName = "farrier-agent"
...
// installing, debugging, reading `farrier-agent facts` — the agent behaves exactly as it does on Linux
