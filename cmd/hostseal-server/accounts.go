...
// accountsCommand implements `hostseal-server accounts`.
//
...
func accountsCommand(argv []string) int {
	...
	default:
		fmt.Fprintf(os.Stderr, "hostseal-server: unknown accounts command %q\n\n", argv[0])
		accountsUsage()
		return 2
	}
}
...
func accountsUsage() {
  ...
  hostseal-server accounts add    [--tenant SLUG | --platform] --email ADDRESS [--name NAME]
  hostseal-server accounts list   [--tenant SLUG | --platform]
  hostseal-server accounts passwd [--tenant SLUG | --platform] --email ADDRESS
  hostseal-server accounts remove [--tenant SLUG | --platform] --email ADDRESS
...
HostSeal for other people should not require being able to see what they run.

The password is asked for on the terminal, never taken as a flag: argv is world-readable in ps.
Every command needs --database, or HOSTSEAL_DATABASE_URL.
`)
}
...
func bindAccountFlags(fs *flag.FlagSet) accountFlags {
	return accountFlags{
		...
		tenant: fs.String("tenant", envOr("HOSTSEAL_TENANT", "default"),
...
func openAccountStore(ctx context.Context, flags accountFlags) (store.Store, accountSide, error) {
	if *flags.dsn == "" {
		return nil, accountSide{}, errors.New("a database URL is required: pass --database or set HOSTSEAL_DATABASE_URL")
	}
	...
	if err != nil {
		...
		return nil, accountSide{}, fmt.Errorf("reading the tenant list: %w\n"+
			"If this database is new, run `hostseal-server serve` once first: it creates the schema and "+
	...
	_ = backing.Close()
	return nil, accountSide{}, fmt.Errorf("no fleet with the slug %q; run `hostseal-server serve` once to "+
...
func accountsAdd(argv []string) int {
	...
	if *flags.email == "" {
		fmt.Fprintln(os.Stderr, "hostseal-server: --email is required")
		return 2
	}
	setupLogging()
	...
	id, err := server.NewID()
	if err != nil {
		fmt.Fprintf(os.Stderr, "hostseal-server: allocating an account id: %v\n", err)
		return 1
	}
	...
	if errors.Is(err, store.ErrConflict) {
		...
		fmt.Fprintf(os.Stderr, "hostseal-server: %s already has an account on this control plane\n", address)
		return 1
	}
	if err != nil {
		fmt.Fprintf(os.Stderr, "hostseal-server: creating the account: %v\n", err)
		return 1
	}
...
func accountsList(argv []string) int {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "hostseal-server: listing accounts: %v\n", err)
		return 1
	}
	if len(accounts) == 0 {
		fmt.Printf("There are no accounts on %s, so nobody can sign in to it.\n", side.label)
		fmt.Println("Create one with `hostseal-server accounts add`.")
		return 0
	}
...
func accountsPasswd(argv []string) int {
	...
	if err := side.scope.UpdateAccountPassword(ctx, account.ID, hash); err != nil {
		fmt.Fprintf(os.Stderr, "hostseal-server: changing the password: %v\n", err)
		return 1
	}
...
func accountsRemove(argv []string) int {
	...
	if !*assumeYes {
		...
		if !ok {
			fmt.Fprintln(os.Stderr, "hostseal-server: not removed.")
			return 3
		}
	}
	if err := side.scope.DeleteAccount(ctx, account.ID); err != nil {
		fmt.Fprintf(os.Stderr, "hostseal-server: removing the account: %v\n", err)
		return 1
	}
...
func findAccount(ctx context.Context, backing store.Store, side accountSide, email string) (store.Account, int) {
	...
	if errors.Is(err, store.ErrNotFound) {
		fmt.Fprintf(os.Stderr, "hostseal-server: no account for %s\n", auth.NormaliseEmail(email))
		return store.Account{}, 1
	}
	if err != nil {
		fmt.Fprintf(os.Stderr, "hostseal-server: reading the account: %v\n", err)
		return store.Account{}, 1
	}
	if account.TenantID != side.tenant {
		...
		fmt.Fprintf(os.Stderr, "hostseal-server: %s does not belong to %s\n", account.Email, side.label)
		return store.Account{}, 1
	}
	return account, 0
}
...
func readNewPassword() (string, int) {
	...
	if string(password) != string(confirmation) {
		fmt.Fprintln(os.Stderr, "hostseal-server: the passwords do not match")
		return "", 1
	}
...
func flushOrFail(w *tabwriter.Writer) int {
	if err := w.Flush(); err != nil {
		fmt.Fprintf(os.Stderr, "hostseal-server: writing the listing: %v\n", err)
		return 1
	}
	return 0
}
