...
#
# The conffiles in /etc/hostseal are removed by dpkg on purge, not here; this only takes the empty
# directory afterwards. State under /var/lib/hostseal goes on purge and not on remove, so that
...
case "$1" in
	purge)
		rm -rf /var/lib/hostseal /var/log/hostseal
		rmdir /etc/hostseal 2>/dev/null || true

		if getent passwd hostseal >/dev/null 2>&1; then
			deluser --system --quiet hostseal >/dev/null 2>&1 || true
		fi
		if getent group hostseal >/dev/null 2>&1; then
			delgroup --system --quiet hostseal >/dev/null 2>&1 || true
		fi
		;;
	remove)
