<#
.SYNOPSIS
    Installs or upgrades the Farrier agent on a Windows Server host.

.DESCRIPTION
    This script is the Windows counterpart of the .deb's postinst, and it is the only PowerShell in
    Farrier. That is a distinction worth stating plainly, because "Farrier does not use PowerShell" is
    ...
    have complete access to system resources", and every act Farrier's guarantee prevents is a cmdlet.
...
# what causes NT SERVICE\farrier-agent to exist. Changing one without the other produces a service that
# cannot start, with an error naming a logon failure rather than a name mismatch.
$ServiceName  = 'farrier-agent'
...
function Assert-Administrator {
    ...
    if (-not $principal.IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) {
        throw 'Install-FarrierAgent must run in an elevated session.'
    }
}
...
function Remove-FarrierService {
    <#
        .SYNOPSIS
...
if (-not $PSCmdlet.ShouldProcess($env:COMPUTERNAME, 'Install the Farrier agent')) { return }
...
$binPath = '"{0}" run' -f (Join-Path $InstallDir 'farrier-agent.exe')
New-Service -Name $ServiceName `
    -BinaryPathName $binPath `
    -DisplayName 'Farrier agent' `
    -Description 'Reports this host to a Farrier control plane. Outbound only; it has no listening port.' `
...
Write-Host @"
The Farrier agent is installed and running as $ServiceAccount.
